...
- CLI arguments now follow `claude` / `pi` conventions. The TUI is the default
  ...
  - `chaz` → TUI (previously launched the Matrix bridge). When the
    `matrix:` block is configured, the Matrix bridge also runs in the
  ...
  - `chaz --no-tui` → headless: Matrix is the only bridge, no TUI.
    Replaces the prior `--matrix` flag.
  - `chaz --no-matrix` → TUI only; suppresses the background Matrix bridge
