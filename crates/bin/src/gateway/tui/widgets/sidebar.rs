...
///
/// Selection marker `>` is rendered on the active row in accent color;
/// inactive rows are dim. Each row occupies one terminal line; if the
/// sidebar is shorter than `items.len()`, the tail is clipped (no scroll
/// in v1 — categories are flat and few).
