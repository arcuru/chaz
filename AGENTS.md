...
**Status**: Active development — Phases 0–16 complete (architecture, tools, security, multi-agent, sessions, scheduling, MCP, tool profiles, structured errors, LLM resilience, gateway command unification, grant config unification, session/transport decoupling). Recent: `SessionBinding`/`transport_id` ripped out; session metadata lives in each session's own eidetica DB (syncs with the session); central registry holds only indices (`sessions`, `matrix_channels`, `session_names`); explicit Matrix channel attachment via `!chaz attach/detach/channels`; startup fan-out installs response callbacks on every persisted channel so scheduled-session replies reach Matrix without an active user. Earlier: typed capability grants, per-agent grant overlays, MCP Streamable HTTP transport, MCP tool directory scanning, dynamic tool re-discovery, loop detection, LLM request timeout, retry with exponential backoff.
...
session.rs           SessionRegistry (index-only: sessions/matrix_channels/session_names DocStores + new-session events) + Session (per-conversation eidetica DB with entries Table + meta DocStore) + EntryType (Message, Directive, ToolCall, ToolResult, Ack, Error, Summary) + SessionMeta (per-session config: name, agent, model, role, backend — lives in the session's own DB so it syncs)
