...
tools/
  ...
  search.rs          web_search — query-to-results (Low risk, approval Never). Holds a `Vec<SearchBackend>` (Tavily/Brave/Serper/DuckDuckGo) configured from `web_search.backends: [...]` as an ordered preference list. `try_backends` loops on any ToolError → fails over to the next entry; empty results do NOT trigger failover (legitimate answer). `try_backends_with` extracts the loop over a closure for unit testing without HTTP. Api_keys resolve via SecretStore under per-entry `secret:web_search.<idx>.api_key` refs. Missing-key entries skipped at startup with warn; empty resulting list gets a DuckDuckGo safety net appended.
