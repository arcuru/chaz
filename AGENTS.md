...
hosted_index.rs      Peer-local index of DBs this peer hosts keys for. One `HostedIndex` type with `HostedEntry { db_id, display_name, pubkey }` serves both agents and memory banks — two DocStores inside the `chazdb` (`agents` and `memory_banks`), chosen at construction via `HostedIndex::agents(chazdb)` / `HostedIndex::memory_banks(chazdb)`. Supports register/unregister/find_by_{id,name,pubkey}/sync_from_bootstrap. Needed because eidetica has no "DBs where key K has permission P" inverse query.
