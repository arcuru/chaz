...
db_registry.rs      Peer-local index of DBs this peer hosts keys for. One `DbRegistry` type with `DbEntry { db_id, display_name, pubkey }` serves both agents and memory banks — two DocStores inside the `chazdb` (`agents` and `memory_banks`), chosen at construction via `DbRegistry::agents(chazdb)` / `DbRegistry::memory_banks(chazdb)`. Supports register/unregister/find_by_{id,name,pubkey}/sync_from_bootstrap. Needed because eidetica has no "DBs where key K has permission P" inverse query.
...
heartbeat.rs         Per-session HeartbeatRule Table (cron + task + target_agent_db_id). HeartbeatRunner polls hosted sessions every 30s; fires due rules whose target agent is in the hosted-agents index. last_fired kept peer-local in the `chazdb` (`heartbeat_last_fired` DocStore) so peers don't coordinate.
...
tools/
  ...
  search.rs          web_search — query-to-results (Low risk, approval Never). Holds a `Vec<SearchBackend>` (Kagi/Tavily/Brave/Serper = keyed REST APIs; Searxng = URL-configured JSON; DuckDuckGo = keyless HTML scrape) configured from `web_search.backends: [...]` as an ordered preference list. `try_backends` loops on any ToolError → fails over to the next entry; empty results do NOT trigger failover (legitimate answer). `try_backends_with` extracts the loop over a closure for unit testing without HTTP. Api_keys resolve via SecretStore under per-entry `secret:web_search.<idx>.api_key` refs. Missing-key entries (or searxng with no `url:`) skipped at startup with warn; empty resulting list gets a DuckDuckGo safety net appended.
...
- **Per-session eidetica DBs**: Each conversation gets its own eidetica Database (entries + meta stores). Sessions are identified globally by their eidetica DB root ID. `SessionRegistry` lives inside the peer-local `chazdb` and holds only indices: `sessions` (all known session IDs + origin tag), `matrix_channels` (Matrix room_id → session_db_id — fan-out supported), `session_names` (name → session_db_id).
- **`chazdb`**: The single peer-local eidetica DB for one chaz instance. Never syncs. Holds every index and piece of bookkeeping: `sessions`, `matrix_channels`, `session_names`, `agents` (hosted Living Agents), `memory_banks` (hosted Memory Bank DBs), `heartbeat_last_fired`, `schedules`, `secrets`. Sync-ful state (conversation history, agent config/memory, bank memory) lives in its own per-entity DBs.
...
- **Co-owned agents (Stage 10)**: `/agent invite <ref> <pubkey> [admin|write|read]` grants another peer `Admin(1)` / `Write(10)` / `Read` on the agent DB's AuthSettings. `/agent revoke-peer` reverses it. `/pubkey` prints this peer's default pubkey so the owner can paste it into `/agent invite`. Each peer keeps its own key — keys never cross the wire; authorization is preseeded by writing pubkeys into the target DB, then the DB syncs via `/agent share` + `/agent import`. Session participation works without extra wiring because `attach_agent_to_session` writes _this peer's_ pubkey into session AuthSettings, so two peers hosting the same agent appear as two distinct authorized writers. `Admin(0)` is the creator's exclusive tier and cannot be granted via invite (true handover requires eidetica support that doesn't exist yet).
- **Memory Banks**: Memory bank = any eidetica DB with a `memory` Table<MemoryEntry> subtree. Standalone `MemoryBankDb`s live under `memory:<display_name>` settings names; an Agent DB doubles as a bank of its own memory for anyone with Read on it. Grants mirror Stage-3 session participation: add the agent's pubkey to the bank's `AuthSettings` with `Read`/`Write(10)` (authoritative) and write a `MemoryBankRef { name, db_id, permission }` row into the agent's `memory_banks` Table (view cache). `/memory grant`/`revoke` keep the two sides consistent via auth-first ordering with best-effort rollback. A `memory_banks` DocStore in the chazdb tracks banks this peer hosts (same shape/rationale as the `agents` index — both are `DbRegistry` instances).
- **Hosted-DB index**: Peer-local `agents` and `memory_banks` DocStores in the chazdb (keyed by DB root ID, value = `{db_id, display_name, pubkey}`). One generic `DbRegistry` type serves both, constructed via `DbRegistry::agents(chazdb)` / `DbRegistry::memory_banks(chazdb)`. The agents index is populated at startup via `sync_from_bootstrap`. Supplies the pubkey→agent lookup for routing (eidetica has no inverse "DBs where key K has permission P" query).
...
- **Heartbeat rules**: `HeartbeatRule { id, cron (6 fields), task, target_agent_db_id }` stored inside each session DB as a Table. `HeartbeatRunner` (background task, 30s poll) scans all sessions the peer knows about, fires rules whose target agent is in the hosted-agents index and whose cron is due vs peer-local `last_fired` (chazdb `heartbeat_last_fired` DocStore). Firing writes a `Directive` entry; the mention-aware router handles turn-taking from there.
