...
tools/
  ...
  search.rs          web_search — query-to-results (Low risk, approval Never). Backend enum (Tavily/Brave/Serper = API key required; DuckDuckGo = keyless HTML scrape fallback). Chosen at startup from `web_search:` config (api_key resolves via SecretStore like LLM backends). Keyless fallback on missing-key so startup never fails on misconfig.
