...
**Status**: Active development — Phases 0–16 + Living Agents Stages 1–4 complete. Recent (Living Agents): agents have persistent eidetica-DB identity signed by per-agent keys; a peer-local `AgentIndex` maps pubkeys to hosted agents in O(1); session AuthSettings is the authoritative participant list (`SessionMeta.agents` mirrors it as a readable cache); `SessionRegistry::{attach,detach}_agent_to_session` grants/revokes `Permission::Write` and logs the attachment into the agent's history store; routing is key-possession-based via `resolve_agent_for_entry` with precedence _override > @mention > host agent (`SessionMeta.host_agent_db_id`) > first authorized > legacy `agent_name` > default_; heartbeat rules live inside each session DB and are fired by a peer-local `HeartbeatRunner` that only triggers for rules targeting agents this peer hosts (`last_fired` is peer-local in central DB); new shared commands `/agent add|remove|list|host` and `/heartbeat add|remove|list`. Earlier: Phase-16 session/transport decoupling (session metadata lives in each session's own eidetica DB, central registry is index-only, explicit Matrix channel attachment via `!chaz attach/detach/channels`); typed capability grants + per-agent overlays; MCP Streamable HTTP + directory scanning + dynamic tool re-discovery; loop detection; LLM request timeout + retry with exponential backoff.
...
- **Agent registry**: YAML-configurable agents with per-agent tool visibility (ScopedTools with transitive narrowing). Bridged to Living Agent DBs by display name.
