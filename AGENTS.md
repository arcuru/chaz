...
**Status**: Active development — Phases 0–16 + Living Agents Stages 1–8 complete. Recent (Stage 8, 2026-04-20): `Server::hydrate_agent_from_db` runs on every message in `process_session` right after `resolve_agent_for_entry` — re-reads the agent's `AgentDb::config`, rebuilds the runtime `Agent` via `AgentRegistry::build_from_db_config` (which resolves role names against a new `AgentRegistry::config_roles` field stashed from yaml `roles:`), and upserts into the registry so `can_spawn` / `default_agent` see fresh state too. Origin-peer config edits propagate to importing peers via eidetica sync, no restart. `/agent new` and `/agent import` switched to `build_from_db_config` for the same role-resolution path. Earlier (Stage 7): per-agent memory migrated from central DB (`memory:<agent_name>`) to each Living Agent's own `AgentDb::memory` store. `Remember`/`Recall` now take `Arc<SessionRegistry>` + `AgentIndex` at construction and resolve the running agent's DB per-call via `SessionRegistry::open_agent_db`. New peer-global scope: `GlobalRemember`/`GlobalRecall` tools write/read a `global_memory` DocStore on central DB. Both scopes gated by a new `MemoryGrant { allow_self, allow_global }` — defaults `allow_self=true`, `allow_global=false`. Main.rs now calls `agent_db::ensure_agent_db` for every `AgentRegistry` entry so the default `chaz` agent gets a DB too. Earlier (Stage 6): Three new shared commands `/agent new <name>` / `/agent share <ref>` / `/agent import <ticket>`. `AgentRegistry` refactored from `Vec<Agent>` to `RwLock<Vec<Agent>>` with a new `register(agent)` method so Living Agents created at runtime land in the routing/permission path immediately. `SessionRegistry::create_new_agent_db` wraps `agent_db::create_agent_db` with the user mutex and rejects duplicate display names; `open_agent_db` returns `Option<AgentDb>` (None when this peer holds no key for the DB). `/agent import` syncs the DB via `DatabaseTicket` then registers locally — read-only import (no key on this peer) fails gracefully; blocked on eidetica's `Database::open_unauthenticated` being `pub(crate)` for true read-only access. Stage 5 (earlier today): `spawn_agent` delegates to Living Agents, new `spawn_task` generates ephemeral revocable keys, both wire parent→child `DelegatedTreeRef { max: Admin(0) }` so parent admins inherit admin on the child transparently. Known gap: task writes aren't cryptographically signed by the ephemeral key (blocked on `User::key_manager()` being `pub(crate)`); see PLAN.md "Eidetica Feedback". Earlier (Stages 1-4): agents have persistent eidetica-DB identity signed by per-agent keys; a peer-local `AgentIndex` maps pubkeys to hosted agents in O(1); session AuthSettings is the authoritative participant list (`SessionMeta.agents` mirrors it as a readable cache); `SessionRegistry::{attach,detach}_agent_to_session` grants/revokes `Permission::Write` and logs the attachment into the agent's history store; routing is key-possession-based via `resolve_agent_for_entry` with precedence _override > @mention > host agent (`SessionMeta.host_agent_db_id`) > first authorized > legacy `agent_name` > default_; heartbeat rules live inside each session DB and are fired by a peer-local `HeartbeatRunner` that only triggers for rules targeting agents this peer hosts (`last_fired` is peer-local in central DB); shared commands `/agent add|remove|list|host` and `/heartbeat add|remove|list`. Earlier: Phase-16 session/transport decoupling (session metadata lives in each session's own eidetica DB, central registry is index-only, explicit Matrix channel attachment via `!chaz attach/detach/channels`); typed capability grants + per-agent overlays; MCP Streamable HTTP + directory scanning + dynamic tool re-discovery; loop detection; LLM request timeout + retry with exponential backoff.
...
212 unit tests covering security (leak detection, SSRF, sanitizer, XML wrapping, rate limiting), context budgeting (tiktoken), agent spawn permissions, tool profiles (globs), secret resolution, loop detection, MCP (SSE parsing, JSON-RPC response handling, tool metadata refresh/removal, config deserialization, directory scanning, subprocess integration lifecycle/error/death), grants (shell/network checks, legacy migration, per-kind merge), Living Agents (Agent DB create/open/idempotent-bootstrap, AgentIndex register/find/sync, SessionMeta.agents round-trip, session attach/detach + auth-and-meta-and-history effects, routing via AuthSettings + legacy fallback, @mention parsing, mention routing, host-agent fallback, override precedence, heartbeat rule CRUD + last_fired persistence, parent→child delegation wiring, ephemeral task-key lifecycle + revoke-without-breaking-reads, AgentRegistry runtime register + dedup, Agent::from_db_config field mapping, create_new_agent_db dup rejection, open_agent_db-None-without-key), memory (per-agent own-memory round-trip, grant-absent/explicit-deny errors, per-agent isolation across peers, global-grant-required, global cross-agent visibility), and live config hydration (build_from_db_config resolves config-defined roles, upsert replace/insert, config-edit pickup, Server::hydrate_agent picks up DB edits + returns input when no DB). Use `CARGO_TARGET_DIR=target-test cargo test --bin chaz`. Build deps: `pkg-config`, `openssl`, `sqlite`.
...
agent_db.rs          Living Agents: AgentDb handle wrapping an eidetica Database with well-known stores (config, memory, meta, history). AgentDbConfig/AgentMeta JSON blobs. create_agent_db generates a fresh per-agent key via User::add_private_key; bootstrap_from_config idempotently materializes one Agent DB per yaml `agents:` entry.
...
tools/
  ...
  memory.rs          remember/recall (agent's own AgentDb::memory), global_remember/global_recall (peer-shared central DB). All capability-gated by MemoryGrant {allow_self, allow_global}; defaults on for self, off for global.
security/
...
- **Memory**: two scopes, both capability-gated by `MemoryGrant { allow_self, allow_global }`. (1) **Own memory** — `remember`/`recall` write to the running agent's `AgentDb::memory` store (Table<MemoryEntry>), resolved per-call via `SessionRegistry::open_agent_db(ctx.agent_name → AgentIndex → db_id)`. Travels with the agent through eidetica sync; naturally isolated. (2) **Global memory** — `global_remember`/`global_recall` write to a `global_memory` Table on central DB, shared across every agent on this peer. Defaults: `allow_self=true` (out-of-box), `allow_global=false` (must opt in via `security.tool_policies.global_remember.grants.memory.allow_global: true` or per-agent overlay).
...
- **Living Agents**: Each agent is its own eidetica Database signed by a per-agent PrivateKey. Bootstrap from yaml: `agent_db::bootstrap_from_config` idempotently creates one Agent DB per `agents:` entry (named `agent:<display_name>`), materializing `config`/`memory`/`meta`/`history` stores. Whoever holds the key hosts the agent. Stage 3+ routing is key-possession-based.
...
- **Command dispatch**: User-facing session commands are parsed to a transport-neutral `commands::Command`, run through `commands::dispatch(cmd, ctx) -> CommandOutcome`, and rendered by the gateway. Both TUI (`/<cmd>`) and Matrix (`!chaz <cmd>`) share the same command set: `sessions`, `info`, `name`, `share`, `sync`, `compact`, `print`, `channels`, `schedules`, `run`, `model`, `role`, `backend`, `backends`, `agent add|remove|list|host|hosted|new|delete|share|import` (+ `agents`), `heartbeat add|remove|list`. Stage 6 added `agent new <name> [k=v ...]` (creates Living Agent DB with optional `role`/`model`/`tools`/`max_iterations`/`tool_profile`/`max_context_tokens` overrides; unknown keys rejected), `agent share <ref>` (generates DatabaseTicket), `agent import <ticket>` (syncs + upserts), `agent hosted` (lists peer-local Living Agents), `agent delete <ref>` (unregisters from index + runtime registry + sweeps heartbeat rules peer-wide; DB preserved for archive; refuses while attached to a session). Detaching an agent via `agent remove` also sweeps session-local heartbeat rules targeting that agent. `CommandContext` carries `{server, scheduler, secrets, backend, session_db_id, session_db, current_agent, session_name, config_roles, default_role}`. `CommandOutcome` variants: `Text` / `Error` / `SessionsList` / `SessionSwitched(Box<…>)` / `Quit`. Transport-specific extras stay in the gateway: TUI has `/debug`, `/raw`, `/clear` (display), picker modal, `/quit`; Matrix has `party`, `rename` (room), `send` (no-context), `clear` (history marker), `approve`/`deny`, plus `attach`/`detach` (which need to install the response callback on mutation).
